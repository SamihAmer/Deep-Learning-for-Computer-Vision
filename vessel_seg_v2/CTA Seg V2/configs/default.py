...
CONFIG = {
    ...
    "num_workers": 0,                        # 0 since volumes are preloaded into memory
    "train_val_split": 0.8,                    # 80/20 random split
    "modality": "ct",                            # "ct", "mr", or "all" -- filter cases by modality

    # ── Patch sampling ────────────────────────────────────────────────────
    "patch_size": (128, 128, 128),             # voxel crop dimensions
    "patches_per_volume": 4,                   # patches sampled per volume per epoch
    ...
    "stratified_eval": True,                   # compute per-vessel-class metrics
    "qualitative_cases": 5,                    # number of cases to render as 3D comparisons

    ...
    def __init__(
        ...
        for i, case in enumerate(case_list):
            try:
                ...
                if (i + 1) % 10 == 0 or i == 0:
                    print(f"  Loaded {i + 1}/{len(case_list)}")
            except Exception as e:
                print(f"  Error loading case {i}: {case['image']}\n    {e}")
                raise
        print("Done.")
...
def discover_cases(data_dir: str, modality: str = "ct") -> List[Dict[str, str]]:
    ...
    for img_path in images:
        ...
        # TopCoW images have _0000 suffix (e.g. topcow_ct_001_0000.nii.gz)
        # but labels don't (e.g. topcow_ct_001.nii.gz) — strip it
    ...
    if modality.lower() == "ct":
        cases = [c for c in cases if "topcow_ct_" in os.path.basename(c["image"])]
    elif modality.lower() == "mr":
        cases = [c for c in cases if "topcow_mr_" in os.path.basename(c["image"])]
    elif modality.lower() != "all":
        raise ValueError(f"Unknown modality '{modality}'. Expected 'ct', 'mr', or 'all'.")

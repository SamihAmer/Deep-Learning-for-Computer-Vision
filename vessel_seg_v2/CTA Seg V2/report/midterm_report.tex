...
The most revealing results emerge from TopBrain fine-tuning (Table~\ref{tab:topbrain}). When evaluated across all 6 clinical vessel groups---encompassing 40 anatomical classes---the Skeleton Recall model dramatically outperforms both alternatives on thin and distal structures:
...
However, the picture changes dramatically when evaluating comprehensive vessel coverage via TopBrain fine-tuning. The Skeleton Recall model's near-zero-to-0.589 improvement on small arteries demonstrates that topology-aware losses create fundamentally different learned representations---ones that generalize better to thin, difficult structures during transfer learning. We hypothesize that the centerline-focused gradient signal during from-scratch training shapes the network's feature hierarchy to better encode tubular geometry at fine scales, even though this benefit is not visible on the limited 13-class evaluation.
...
We presented a controlled ablation study comparing three loss configurations for cerebral CTA vessel segmentation. While the Dice+CE baseline achieves the best aggregate metrics on the 13-class TopCoW benchmark, the Skeleton Recall loss demonstrates dramatically superior performance when fine-tuned for comprehensive vessel coverage, achieving 0.589 DSC on small arteries where both alternatives score near zero. This suggests that topology-aware losses create learned representations that better generalize to thin tubular structures. The large-versus-small vessel gap of $>$0.33 DSC across all configurations highlights the ongoing challenge of communicating artery segmentation. Future work will explore wider HU windows to reduce false positives on bone, class-balanced sampling strategies, and retraining on corrected labels from our model-assisted annotation pipeline.
